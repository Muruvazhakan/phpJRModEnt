<?php include '../DBconfig.php';	
	...
	$result = $conn->query("SELECT * FROM Header_Details hd join Image_Count ic on hd.Header_Details_id = ic.Image_Header_Id where hd.topLine='$headername' and ic.Image_Url_No='$imgid'  ");
...
if ($result->num_rows > 0) {
	...
	if ($result->num_rows == 0) {
		$result3 = $conn->query("SELECT * FROM Header_Details hd where hd.topLine='$headername' ");
		if ($result3->num_rows > 0) {
			echo  json_encode('NoImage'); // 
		}
		else{
			echo  json_encode('NoDetails'); // 
		}		
	} else {
